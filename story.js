// Five vector scenes are blended by scroll position. No image or WebGL dependency.
(() => {
  const home = document.getElementById('home');
  if (!home) return;
  const reduced = matchMedia('(prefers-reduced-motion: reduce)').matches;
  const svg = (body, defs = '') => `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 760 760" fill="none" aria-hidden="true"><defs>
  ...
  const rock = svg(`
    ...
    <text x="116" y="187" fill="#cdb4cb" font-family="monospace" font-size="11" letter-spacing="4">SPECIMEN / 01</text>
    <text x="548" y="635" fill="#cdb4cb" font-family="monospace" font-size="11" letter-spacing="3">CORE →</text>
  `, `<clipPath id="rockClip"><path d="M190 430 235 286 338 214 463 179 582 249 626 374 571 522 433 594 285 563 209 501Z"/></clipPath>`);
  ...
  const mineral = svg(`
    ...
    <text x="116" y="187" fill="#d7bddd" font-family="monospace" font-size="11" letter-spacing="4">CRYSTAL / 02</text>
  ...
  const palette = ['#746b9b','#cb99b7','#92c4c5','#b9b7d4','#e3b8aa','#4c8291','#a6a4c4','#dc9abb','#78a6a7'];
  const points = Array.from({length: 11}, (_, y) => Array.from({length: 11}, (_, x) => [105 + x * 55 + (x && x < 10 ? (hash(x * 31 + y * 13) - .5) * 20 : 0), 105 + y * 55 + (y && y < 10 ? (hash(x * 37 + y * 43) - .5) * 20 : 0)]));
  let grains = '';
  for (let y = 0; y < 10; y++) for (let x = 0; x < 10; x++) {
    const p = [points[y][x], points[y][x+1], points[y+1][x+1], points[y+1][x]];
    const poly = p.map(v => v.map(n => n.toFixed(1)).join(',')).join(' ');
    const c = palette[Math.floor(hash(y * 71 + x * 29) * palette.length)];
    grains += `<polygon points="${poly}" fill="${c}" stroke="#182d42" stroke-width="3"/>`;
    if (hash(x * 11 + y * 17) > .61) {
      const a = p[0], b = p[2];
      grains += `<path d="M${a[0]+9} ${a[1]+14} ${b[0]-8} ${b[1]-12}" stroke="#fff0f6" stroke-opacity=".5" stroke-width="3"/>`;
    }
  }
  const microscope = svg(`
    ...
    <g clip-path="url(#lensClip)">${grains}
      <path d="M70 428Q251 171 399 371T691 240" stroke="#fff8ef" stroke-opacity=".45" stroke-width="16"/>
      <path d="M120 506Q323 236 446 465T692 373" stroke="#3c6f8e" stroke-opacity=".5" stroke-width="21"/>
    ...
    <text x="556" y="173" fill="#e7f4f6" font-family="monospace" font-size="11">XPL / 03</text>
  `, `<clipPath id="lensClip"><circle cx="380" cy="380" r="237"/></clipPath><radialGradient id="lensShade"><stop offset=".35" stop-color="#fff" stop-opacity=".04"/><stop offset="1" stop-color="#061a2b" stop-opacity=".63"/></radialGradient>`);
  ...
  const arts = [rock, mineral, microscope, atom, network];
  const labels = ['岩石 / ROCK', '矿物 / MINERAL', '显微镜下 / THIN SECTION', '原子 / ATOM', '抽象点线 / DATA FIELD'];
